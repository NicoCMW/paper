...
1. Confirm that the Canvas MCP tools are available in the current task, especially `canvas_get_state`, `canvas_get_selection`, and `canvas_receive_generated_asset`. Use `canvas_list`, `canvas_create`, `canvas_switch`, and `canvas_rename` when the user asks to manage named Canvases.
...
2. Respect requested dimensions for the generated bytes. If the user asks for 16:9, use a 16:9 output such as 1536×864 or 1024×576. The local Canvas adapter normalizes the displayed Asset footprint to the largest selected Reference, so a high-resolution generation does not appear enormous on the Canvas.
3. After generation, call `canvas_receive_generated_asset` with:
   ...
   The width and height are generation dimensions; the adapter owns the smaller local display dimensions.
