...
**Note**:
Editable text placed directly on a Canvas. A Note is spatial, persistent, and independently selectable, with its own font size, text color, background (including transparent), and left/center/right alignment. Its rectangle can be resized from corners or edges; it is not an Asset, Board, or comment.
